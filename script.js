// script.js com gerador de questões de alternativas diárias + modo escuro + timer
...
function getDataAtualGMT3() { const now = new Date(); const op = { timeZone: 'America/Sao_Paulo' }; return now.toLocaleDateString('en-CA', op); }
...
function mostrarQuestaoDoDia() { const index = getIndiceTema(); const tema = temas[index]; const { text, alternatives, answer } = geradorestema; document.getElementById("question").textContent = text;
